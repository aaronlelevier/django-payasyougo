...
class Hotel(AbstractBase):
    """
    Main Account Model that Business accounts will be under and supporting cost 
    Models will link to.
    """
    # Required
    ...
    address_zip = models.PositiveIntegerField(_("Zipcode"),
...
class AcctStmtManager(models.Manager):
    ...
    def get_or_create(self, hotel, month=timezone.now().month, year=timezone.now().year):
        ...
        values = {
            'total_sms': total_sms,
            'monthly_costs': 0.04 * total_sms, # TODO: make this a sum() aggregation from AcctTrans
